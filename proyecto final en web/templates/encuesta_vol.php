...
$id_voluntario = isset($_SESSION['id_voluntario']) ? $_SESSION['id_voluntario'] : null;

// Verifica si el usuario está logueado como voluntario
if ($id_voluntario) {
    $id_voluntario = $_SESSION['id_voluntario']; // Si es un voluntario, toma su ID
} else {
    $id_voluntario = null; // Si no es voluntario, asigna null
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $query = "INSERT INTO encuesta_voluntario (id_voluntario, trabajo, fecha_inicio, fecha_fin, 
    ...
    if ($stmt) {
        $stmt->bind_param("issssssssss", $id_voluntario, $trabajo, $fecha_inicio, $fecha_fin, 
                          $trato_usuario, $trato_organizacion, $trato_voluntario, 
                          $cumplimiento_horarios, $cumplimiento_objetivos, 
                          $satisfaccion_usuario, $comentarios);
        ...
        if ($stmt->execute()) {
            ...
            echo "<script>
                    setTimeout(function() {
                        window.location.href = 'dashboard_voluntario.php'; // Redirige a dashboard_voluntario
                    }, 3000);
                  </script>";
        } else {
